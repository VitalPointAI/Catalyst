...
export default class FungibleTokens {
    ...
    static async getLikelyTokenContracts({ accountId }) {
        return await axios.get(`${ACCOUNT_HELPER_URL}/account/${accountId}/likelyTokens`);
    }
    ...
    static async getMetadata( contractName ) {
        ...
        return await account.viewFunction(contractName, 'ft_metadata');
    }

    static async getBalanceOf( contractName, accountId ) {
        let account = wallet.getAccountBasic('groovy')
        return await account.viewFunction(contractName, 'ft_balance_of', { account_id: accountId });
    }
