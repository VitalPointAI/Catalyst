...
export default function MemberProposalTable(props) {
    ...
    const { memberProposalList, eventCount, matches, accountId, loaded, contract } = props
    ...
    await contract.processProposal({
        proposalIdentifier: proposalIdentifier
        }, process.env.DEFAULT_GAS_VALUE)
    };

    async function handleCancelAction(proposalIdentifier) {
        await contract.cancelProposal({
            proposalIdentifier: proposalIdentifier
            }, process.env.DEFAULT_GAS_VALUE)
        };
    ...
    async function getStatus(proposalIdentifier) {
        ...
        let flags = await contract.getProposalFlags({proposalIdentifier: proposalIdentifier})
    ...
    async function getVotingPeriod(proposalIdentifier) {
        return await contract.isVotingPeriod({proposalIdentifier: proposalIdentifier})
     }

     async function getGracePeriod(proposalIdentifier) {
        return await contract.isGracePeriod({proposalIdentifier: proposalIdentifier})
     }
 

     async function getUserVote(proposalIdentifier) {
        let result = await contract.getMemberProposalVote({memberAddress: accountId, proposalIdentifier: proposalIdentifier})
